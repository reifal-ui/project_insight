...
export default function UserDropdown() {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        if (!response.ok) {
          // Token invalid, hapus dan set user null
  ...
  // Jika tidak ada user (belum login), tampilkan tombol Sign In
  if (!user) {
    return (
      <div className="relative">
        <Link
          to="/TailAdmin/signin"
          className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-full hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-700"
        >
          <svg
            width="18"
            height="18"
